...
from src.data.ablation2_dataloader import CoronaryArteryDataModule
...
@click.option(
    "--loc",
    type=click.Choice(["decoder", "encoder", "both"]),
    default="decoder",
    help="Location of the feature modulation block (decoder, encoder, both).",
)
def main(
    arch_name,
    loss_fn,
    ...
    # Determine architecture based on loc parameter
    if loc == "decoder":
        arch_name = arch_name
    elif loc == "encoder":
        arch_name = "EncoderSPADE" + arch_name
    elif loc == "both":
        arch_name = "SPADE" + arch_name

    # set up loggers and checkpoints
    log_dir = f"result/ab2_{arch_name}" + f"_{loc}"
    ...
    data_module = CoronaryArteryDataModule(
        data_dir="data/imageCAS_ablation",
        batch_size=1,
        patch_size=(96, 96, 96),
        num_workers=4,
        cache_rate=0,
